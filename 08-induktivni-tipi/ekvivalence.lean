...
theorem trd4 {A : Type} {xs ys zs : List A} :
  concat (concat xs ys) zs =
...
theorem trd5 {A : Type} {xs ys : List A} :
...
theorem map_assoc {A B C : Type} {f : A → B} {g : B → C} {xs : List A} :
  map g (map f xs) =
  map (g ∘ f) xs :=
  by
    induction xs with
...
theorem map_concat {A B : Type} {f : A → B} {xs ys : List A} :
  map f (concat xs ys) =
  concat (map f xs) (map f ys) :=
  by
    induction xs with
    | nil =>
      simp [map, concat]
...
theorem map_reverse {A B : Type} {f : A → B} {xs : List A} :
  map f (reverse xs) =
  reverse (map f xs) :=
  by
    induction xs with
...
def mirror {A : Type} : tree A → tree A :=
  fun t =>
    match t with
    | tree.empty => tree.empty
    | tree.node n l r => tree.node n (mirror r) (mirror l)
...
theorem mirror_depth {A : Type} {t : tree A} : depth (mirror t) = depth t :=
  by
    ...
    | empty =>
      simp [mirror]
    | node a al ar ihl ihr =>
      simp [depth, mirror ,ihl, ihr, max_comm]
